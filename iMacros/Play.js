...
function Player() {
	...
	this.play = function(fileNameOrUrl) {
		...
		if (script === 'undefined' 
			|| script === '' 
			|| script.match('Error 404') 
			|| script.match('DOCTYPE')
			|| script.match('Not Found')) {
...
=======
  ...
  * @param  {String}  folder   save to folder
  * @param  {String}  fileName file name
  * @param  {String}  fileType file type TYPE=(CPL|MHT|HTM|TXT|EXTRACT|BMP|PNG|JPEG)
 ...
 function saveAs(folder, fileName, fileType) {
 	...
 	return player.playMacroLine('SAVEAS TYPE=' + fileType.toUpperCase() + ' FOLDER=' + folder + ' FILE=' + fileName);
 }
